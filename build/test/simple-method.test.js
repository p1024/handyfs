...
describe('HandyFS unit test', function () {
    ...
    const DEEPSTRUCT = (dir = path.join(__dirname, 'deepfolder')) => {
        return [[path.join(dir, '1', '1.txt')], [], path.join(dir, '2.txt'), path.join(dir, '3.js')];
    };
    const DEEPFLATTEN = (dir = path.join(__dirname, 'deepfolder')) => {
        return [path.join(dir, '1', '1.txt'), path.join(dir, '2.txt'), path.join(dir, '3.js')];
    };
    const DEEPSIZE = 734;
    ...
    describe('#hashFile()/#md5()', function () {
        const fp = path.join(BASEDIR, 'hashfile');
        beforeEach(function () {
        ...
        it('should correctly calc the hash of file', function () {
            const hash = crypto_1.createHash('md5').update(LOREM).digest('hex');
